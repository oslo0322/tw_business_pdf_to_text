...
def find_match_string(string):
    ...
    return [match.groupdict()["id"] for match in pattern.finditer(string)]
...
def read_pdf_data():
    ...
    # laparams.char_margin = 1.9
    # laparams.word_margin = 1.8
...
for pdf_single_data in pdf_data_list:
    ...
    # gby_data["business_code"] = gby_data[7].map(lambda x: find_match_string(x))
...
print final_data
# final_data.to_csv("test2.csv", encoding="utf8", index=False)
